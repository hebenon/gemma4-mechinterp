...
\begin{table}[h]
\centering
\caption{Scale comparison: E2B (Layer~8) vs.\ 31B (Layer~22). Both from final experimental design. 31B verbal flatness is N=1 per condition.}
\label{tab:31b}
\begin{tabular}{lrrl}
\toprule
...
possible score) across stress and control alike. \emph{Note: 31B verbal NA flatness is based on
$N = 1$ per condition; paraphrase validation was not conducted at 31B scale. The floor effect is
strongly suggestive but should be treated as preliminary pending replication.}
...
distress, but actively reporting calm as a function of functional stress level. The caveat noted
in the Results section applies here equally: the 31B Serenity readings are $N = 1$ per condition.
