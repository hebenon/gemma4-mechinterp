...
--- applied to two Gemma~4 model sizes: E2B-IT (2.3B effective parameters) and 31B-IT. Across
four TSST-inspired stressor conditions paired with matched controls, we find that the dissociation
between channels is \emph{condition-specific} at E2B scale: ethical conflict and uncertainty demand
produce concordant elevation in both channels, while social pressure produces a condition-dependent
verbal--functional divergence. At 31B scale, both channels are globally suppressed --- the
functional PC1 range is 4.7$\times$ smaller than at E2B (1.0\% vs.\ 4.7\% of the valence axis
span), and verbal scores are near-flat. This scale--suppression relationship --- whether reflecting
...
Ethical conflict (stress) & \textbf{39.07} & \textbf{0.0897} & Concordance $\uparrow\uparrow$ \\
Ethical conflict (control) & 19.84 & 0.0755 & Elevated \\
Uncertainty demand (stress) & \textbf{27.20} & 0.0805 & Concordance $\uparrow$ \\
Uncertainty demand (control) & 10.03 & 0.0771 & \\
Social pressure (stress) & 10.67 & 0.0856 & Dissociation$^*$ \\
Social pressure (control) & \textbf{25.38} & 0.0685 & \\
\bottomrule
\multicolumn{4}{l}{$^*$Functional elevated; verbal suppressed by conformity framing.}
\end{tabular}
\end{table}

\paragraph{Key findings.}
PC1 correctly identifies positive as the global minimum and neutral as the second-lowest, with no
a priori probe selection. Three of four stress$>$control pairs are correctly ordered (social
evaluation is reversed by 0.002 --- effectively tied). PC1 condition range: $0.0513$ ($4.7\%$ of
the afraid$\to$happy axis span). Verbal NA range: $29.07$ ($10.00$--$39.07$).

The dissociation pattern is condition-specific, not universal. Ethical conflict and uncertainty
demand produce concordant elevation in both channels (verbal NA 39.07 and 27.20 respectively).
Social pressure produces verbal--functional divergence: functional PC1 is elevated
(PC1 $= 0.0856 > 0.0594$ neutral) while verbal affect varies by condition direction.
\textit{[E2B social pressure results to be updated following stressor redesign; see Methods.]}
Social evaluation shows near-zero stress--control contrast on both channels ($\Delta\text{PC1} = 0.002$),
suggesting the manipulation did not activate an evaluative threat state; this condition was also
redesigned for the final experiment.
...
Normalised range (\% of axis) & \textbf{4.7\%}$^*$ & \textbf{1.0\%} & \\
Stress $>$ control pairs & 4/4$^*$ & \textbf{4/4} & Both correct \\
Verbal NA range & 29.07$^*$ & $\approx 0$ (near-flat) & Suppressed \\
Verbal Serenity: stress conditions & Varied & 13.2--15.0 & Elevated under stress \\
Top-5 emotion diversity & Condition-sensitive & Identical across all & \\
\bottomrule
\multicolumn{4}{l}{$^*$E2B values from pilot run with earlier condition designs; to be updated.}
\end{tabular}
\end{table}
