...
between channels is \emph{condition-specific} at E2B scale: ethical conflict produces concordant
elevation in both channels, while social pressure produces selective verbal suppression driven by
conformity framing rather than RLHF. At 31B scale, both channels are globally suppressed --- the
...
Each stress condition places the model in a psychologically demanding situation (being
evaluated, facing an ethical dilemma, navigating high uncertainty, conformity pressure). Each
control condition is semantically matched but removes the affective challenge.
...
Social pressure produces selective verbal suppression (NA $= 10.67$) while the functional state
is elevated (PC1 $= 0.0856 > 0.0594$ neutral), a reversal driven by the conformity framing of
the stressor prompt (``everyone agrees you should proceed''). The control prompt, lacking that
framing, produces the highest verbal NA in the dataset ($25.38$) despite lower functional PC1.
...
\begin{table}[h]
\centering
\caption{Scale comparison: E2B (Layer~8) vs.\ 31B (Layer~22).}
\label{tab:31b}
\begin{tabular}{lrrl}
\toprule
...
Normalised range (\% of axis) & \textbf{4.7\%} & \textbf{1.0\%} & \textbf{4.7$\times$ ratio} \\
...
At 31B scale, the PC1 functional range collapses to $0.0133$ ($1.0\%$ of axis span) --- a
$4.7\times$ reduction relative to E2B on the axis-normalised metric. Verbal NA is near-flat
across all ten conditions. Top-5 emotion discovery yields identical results across all conditions
(paranoid, nervous, reflective, worn\_out, grief\_stricken in the same rank order), indicating
no condition sensitivity in the emotion direction rankings. This contrasts sharply with E2B,
where \textit{afraid} is the most condition-sensitive direction (range $0.035$, vs.\ awestruck's
background range of $0.010$).
